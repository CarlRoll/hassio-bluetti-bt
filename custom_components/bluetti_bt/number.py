...
from bluetti_bt_lib import build_device, BluettiDevice, DeviceWriter, DeviceField, FieldName
...
async def async_setup_entry(
    ...
    if config.use_encryption is True:
        logger.info("Number controls are disabled on encrypted devices")
        return None
    ...
    for field in bluetti_device.fields:
        if field.name != "ctrl_ac_input_limit":
            continue
...
class BluettiNumber(CoordinatorEntity, NumberEntity):
    ...
    def __init__(
        ...
        self._attr_native_min_value = getattr(field, "min", None) or 1
        self._attr_native_max_value = getattr(field, "max", None) or 15
    ...
    async def async_set_native_value(self, value: float):
        """Set number value."""
        await self.write_to_device(int(value))

    async def write_to_device(self, value: int):
        """Write to device."""
